...
$user = PermissionMiddleware::requireModule(MODULE_ORDERS); // Or a new module
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!AuthController::checkCsrf($_POST['csrf_token'] ?? null)) {
        ...
        try {
            if ($action === 'create') {
                VoucherModel::create($_POST, $storeId);
                $_SESSION['flash_success'] = 'Đã tạo mã khuyến mãi thành công.';
                header('Location: /store/vouchers.php');
                exit;
            } elseif ($action === 'delete') {
                $id = (int) ($_POST['id'] ?? 0);
                VoucherModel::deleteStoreVoucher($id, $storeId);
...
$vouchers = VoucherModel::getStoreVouchers($storeId);
...
<head>
    ...
    <title>Khuyến mãi - Cửa hàng</title>
    ...
    <main class="portal-shell portal-wide">
        ...
        <section class="portal-panel">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h1 style="margin: 0;">Quản lý mã khuyến mãi</h1>
